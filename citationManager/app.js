...
config.templates.add('my-ieee', csldata);
...
document.getElementById("citationform").addEventListener("submit", async function(e) {
  ...
  const checkbox = document.getElementById("harukite-ikou!");
  if(checkbox.checked){
 
  const audio = new Audio('/citationManager/5ZMy8zWT-Harikitte-Ikou-Kitasan-Black-.mp3');
  ...
  const doiOrUrl = document.getElementById("doi").value.trim();
  const isDoi = doiOrUrl.includes('10.') || doiOrUrl.startsWith('doi');
  ...
  let data = {
    ...
    'container-title': document.getElementById("container").value,
    ...
    DOI: document.getElementById("doi").value.includes('10.') ? document.getElementById("doi").value : undefined,
    URL: !document.getElementById("doi").value.includes('10.') ? document.getElementById("doi").value : undefined
  };
  ...
  try {
    ...
    const finalOutput = result.replace(/^\[\d+\]\s*/, '');
    await db.collection("citations").add({
        formattedText: finalOutput,
    ...
    console.error(err);
    document.getElementById("add").textContent = "Error generating citation.";
  }
});

db.collection("citations").orderBy("createdAt", "desc").onSnapshot((snapshot) => {
  container = document.getElementById("references");
  ...
  snapshot.forEach((doc) => {
    ...
    div.innerHTML = `
      <div style="display: flex; justify-content: space-between; border-bottom: 1px solid #ddd; padding: 10px;">
        <p style="margin: 0;">${item.formattedText}</p>
        <button onclick="deleteCitation('${id}')" style="color: red; border: none; background: none; cursor: pointer;">Delete</button>
      </div>
    `;
...
// --- PART C: DELETE FUNCTION ---
window.deleteCitation = async function(id) {
  if (confirm("Delete this citation?")) {
    await db.collection("citations").doc(id).delete();
  }
};
...
function showImageTemporarily() {
    // Get the image element
    var imageElement = document.getElementById('tempImage');

    // Show the image
    imageElement.style.display = 'block';

    // Set a timer to hide the image after 3 seconds (3000 milliseconds)
    setTimeout(function() {
